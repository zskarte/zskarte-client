...
export class SidebarPrintComponent {
  ...
  private generateQrCodeDataUrl(relativePath: string): Promise<string> {
    ...
    const width = (this.qrCodeSize * this.dpi) / MM_PER_INCHES;
    return QRCodeToDataURL(fullpath, {
      width,
      type: 'image/png',
      margin: width < 80 ? 1 : undefined /* = default */,
    }).catch((err) => {
    ...
    const generateCallback = async () => {
      try {
        ...
        if (mapContext) {
          ...
          mapCanvas.width = Math.round((this.dimensions[0] * this.dpi) / MM_PER_INCHES);
          mapCanvas.height = Math.round((this.dimensions[1] * this.dpi) / MM_PER_INCHES);
          ...
          Array.prototype.every.call(document.querySelectorAll('canvas.ol-layer, .ol-layer canvas'), (canvas) => {
            SidebarPrintComponent.drawCanvas(canvas, mapContext);
            //break after first if emptyMap
  ...
  getTileLoadProgressCallback() {
    let tileCount = 0;
    let tileDone = 0;
    const tileEventCallback = (event: Event) => {
      if (event.type === 'tileloadstart') {
        tileCount++;
  ...
  addAttributions(pdf: jsPDF) {
    ...
    if (this.attributions && this.attributions.length > 0) {
      const attribution = this.attributions.map((a) => a.replace(/<[^>]+>/g, '')).join(' | ');
      pdf.setDrawColor(0);
