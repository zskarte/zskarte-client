...
import { DEFAULT_COORDINATES, DEFAULT_ZOOM, ZOOM_0_RESOLUTION } from './default-map-values';
    ...
    this._session.pipe(skip(1)).subscribe(async (session) => {
      ...
      if (session?.jwt) {
        ...
        if (session.operation?.id) {
          ...
          if (displayState) {
            //if there is state in session apply queryParams before set
            SessionService.overrideDisplayStateFromQueryParams(displayState, queryParams);
            this._state.setDisplayState(displayState);
          } else {
            //if there is no state in session use updateDisplayState to apply queryParams
  ...
  private static overrideDisplayStateFromQueryParams(displayState: IZsMapDisplayState, queryParams: Params) {
    ...
    if (queryParams['size']) {
      try {
        ...
        displayState.mapZoom = Math.log2(ZOOM_0_RESOLUTION) - Math.log2(Math.max(xResolution, yResolution));
      } catch (ex) {
