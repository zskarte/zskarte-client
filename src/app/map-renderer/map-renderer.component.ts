...
import { DEFAULT_COORDINATES, DEFAULT_ZOOM, MM_PER_INCHES } from '../session/default-map-values';
...
export class MapRendererComponent implements AfterViewInit {
  ...
  public ngAfterViewInit(): void {
    ...
    this._map.getLayers().on(['add', 'remove'], () => {
      this._state.updatePrintState((draft: IZsMapPrintState) => {
        draft.attributions = this._map.getAllLayers().flatMap((layer) => layer.getAttributions(this._view));
      });
    });
    ...
    this._state
      ...
      .subscribe((printState) => {
        ...
        this.updatePrintViewInterctions(printState);
  ...
  private updatePrintViewInterctions(printState) {
  ...
  private updatePrintViewCallbacks(printState) {
    if (printState.printView && printState.generateCallback && !this._disabledForPrint) {
      ...
      if (printState.tileEventCallback) {
        (this._mapLayer.getSource() as TileSource)?.on(['tileloadstart', 'tileloadend', 'tileloaderror'], printState.tileEventCallback);
        //auto remove tileEventCallback after render complete / generateCallback callen
        this._map.once('rendercomplete', () => {
          if (printState.tileEventCallback) {
            (this._mapLayer.getSource() as TileSource)?.un(['tileloadstart', 'tileloadend', 'tileloaderror'], printState.tileEventCallback);
          }
        });
      }
