...
import { DEFAULT_COORDINATES, DEFAULT_ZOOM, DEFAULT_DPI, ZOOM_0_RESOLUTION, INCHES_PER_METER } from '../session/default-map-values';
  ...
  public setMapZoomScale(scale: number, dpi?: number) {
    this.updateDisplayState((draft) => {
      if (mercatorProjection) {
        ...
        draft.mapZoom = Math.log2(ZOOM_0_RESOLUTION) - Math.log2(scale / desiredResolution);
      }
    });
  }
